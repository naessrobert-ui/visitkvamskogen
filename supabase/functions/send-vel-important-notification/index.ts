...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const existingResponse = await fetch(`${supabaseUrl}/rest/v1/vel_notifications?case_id=eq.${encodeURIComponent(caseId)}&notification_key=eq.${encodeURIComponent(notificationKey)}&select=id&limit=1`, { headers: serviceHeaders });
    const existing = existingResponse.ok ? await existingResponse.json() : [];
    if (existing.length) return json({ ok: true, alreadySent: true });
    ...
    const results = await Promise.all(members.map((member) => fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { Authorization: `Bearer ${resendApiKey}`, 'Content-Type': 'application/json; charset=utf-8' },
      body: JSON.stringify({ from: fromEmail, to: member.email, subject: `Viktig sak: ${caseItem.title}`, html }),
    })));
    if (results.some((response) => !response.ok)) return json({ error: 'En eller flere e-poster kunne ikke sendes.' }, 502);

    const logResponse = await fetch(`${supabaseUrl}/rest/v1/vel_notifications`, {
      method: 'POST',
      headers: { ...serviceHeaders, 'Content-Type': 'application/json', Prefer: 'return=minimal' },
      body: JSON.stringify({ case_id: caseId, notification_key: notificationKey, recipient_count: members.length }),
    });
    if (!logResponse.ok && logResponse.status !== 409) return json({ error: 'E-posten ble sendt, men loggen kunne ikke oppdateres.' }, 500);
    return json({ ok: true, recipients: members.length });
  } catch (error) {
